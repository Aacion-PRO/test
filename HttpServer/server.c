...
{	
	...
	// 因为需要的数据都在请求行中
	// - 客户端向服务器请求都是静态资源，请求的资源内容在亲求行的第二部分
	// - 只需要将请求完整的保存下来就可用，请求行后边请求头和空行
	...
	//printf("%s\n",buf);
	// 循环结束 -> 读完了
	// 读操作是非阻塞的，当前缓存中没有数据返回-1，errno == EAGAIN
	if(len == -1 && errno == EAGAIN){
		// 将请求行从接收的数据中拿出来
		// 在http协议中换行使用的是 /r/n
		// 遍历字符串，当遇到第一个/r/n的时候意味着请求行就拿到了
		char* pt = strstr(buf,"\r\n");
		// 请求行字节数(长度)
		int reqlen = pt - buf;
		// 保留请求行
...
int parseRequestLine(int cfd,const char* reqLine)
{
	...
	// 需要在程序中判断得到文件的属性 - stat()
	// 判断path中存储的到底是什么字符串？
	char* file = 0;//file中保存的文件路径就是相对路径

	// 如果文件名中有中文，需要还原
	decodeMsg(path, path);

	if (strcmp(path, "/") == 0) {
		// 访问的是服务器资源根目录 /home/robin/luffy
		// / 不是系统根目录，是服务器提供的资目录 == 传递进行的 /home/robin/luffy
		// 如何在服务器将服务器的资源根目录描述出来？
	...
	if (S_ISDIR(st.st_mode)) {
		// 遍历目录，将目录的内容发送给客户端
		// 5. 客户端请求的名字是一个目录，遍历目录发送文件内容给到客户端
...
/*
	status:状态码
	descr:状态描述
	type:Content-Type 的值，要回复数据的类型
	length:Content-length 的值,要回复数据的长度
*/ 
...
/*
	客户端访问目录，服务器需要遍历当前目录，并且将目录中的所有文件发送给客户端
